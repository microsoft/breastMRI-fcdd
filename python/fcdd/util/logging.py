...
class Logger(object):
    ...
    def add_scalar(self, name: str, value: float, step: int = None):
        ...
        WARNING: The execution time grows with the number of recorded values. This should not be used as in tensorboard
        but rather to log average scores at the end of an epoch.
        ...
        if name not in self._scalars:
            ...
            if len(self._scalars[name]) > 1000:
                self.warning(
                    f'Logger did not add scalar for {name} as there are already too many recorded scalars',
                    unique=False
                )
                return
        if step is None:
