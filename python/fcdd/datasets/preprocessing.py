    ...
    def __multi_apply(self, imgs: List, t: Callable):
        if isinstance(t, transforms.RandomCrop):
            ...
            for idx, img in enumerate(imgs):
                ...
                t, (
                    transforms.Resize, transforms.Lambda, transforms.ToPILImage,
                    transforms.ToTensor, BlackCenter, transforms.Normalize, transforms.CenterCrop
                )
        ):
            for idx, img in enumerate(imgs):
...
class ImgTransformWrap(ImgGtTransform):
    """ Wrapper for some transformation that is used in a MultiCompose, but is only to be applied to one of the input s"""
    def __init__(self, t: Callable, k: int = 0):
        self.t = t
        self.k = k
        assert k in (0, 1)

    def __call__(self, img, gt):
        return (self.t(img), gt) if self.k == 0 else (img, self.t(gt))
