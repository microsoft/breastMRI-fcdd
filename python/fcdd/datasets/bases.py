...
class TorchvisionDataset(BaseADDataset):
    ...
    def __init__(self, root: str, logger=None, online_supervision: bool = True):
        super().__init__(root, logger=logger)
        self.online_supervision = online_supervision
    ...
    def loaders(self, batch_size: int, shuffle_train=True, shuffle_test=False, num_workers: int = 0,
        ...
        if validation_ratio > 0.0:
            if validation_ratio > 1.0:
                raise ValueError(f'Validation ratio is {validation_ratio} but needs to be in [0, 1].')
            if not self.online_supervision:
                raise ValueError(f'Validation split requires an online supervisor to be stratified!')
            if len(set(self.train_set.indices)) != len(self.train_set.indices):  # semi-supervision custom data
                if not hasattr(self, 'balance_dataset'):  # hack to id custom ds bc only custom data has this right now.
                    raise ValueError(f'Found duplicate indices in the training set, but the dataset is not custom!')
                unique_subset_indices = np.asarray(sorted(list(set(self.train_set.indices))))
                anomaly_labels = np.asarray(self.train_set.dataset.anomaly_labels)[unique_subset_indices]
                train_indices, val_indices = train_test_split(
                    unique_subset_indices, test_size=validation_ratio, stratify=anomaly_labels
                )
                train_indices = np.asarray(train_indices)
                nominal_mask = np.asarray(anomaly_labels[train_indices]) == self.nominal_label
                anomaly_mask = np.asarray(anomaly_labels[train_indices]) != self.nominal_label
                if anomaly_mask.sum() > 0:
                    train_set = type(self.train_set)(
                        self._train_set.dataset, np.concatenate([
                            train_indices[np.argwhere(nominal_mask).flatten().tolist()],
                            train_indices[
                    ...
                    train_set = type(self.train_set)(self._train_set.dataset, train_indices)
                val_set = type(self.train_set)(self._train_set.dataset, val_indices)
            else:
                train_indices, val_indices = train_test_split(self.train_set.indices, test_size=validation_ratio)
                train_set = type(self.train_set)(self._train_set.dataset, train_indices)
                val_set = type(self.train_set)(self._train_set.dataset, val_indices)
            val_loader = DataLoader(dataset=val_set, batch_size=batch_size, shuffle=shuffle_test,
    ...
    def preview(self, loader, percls=20) -> torch.Tensor:
        """
        ...
        x, y, gts, ref, out = torch.FloatTensor(), torch.LongTensor(), torch.FloatTensor(), torch.FloatTensor(), []
        if isinstance(self.train_set, GTMapADDataset):
            for xb, yb, gtsb in loader:
                ...
                if all([x[y == c].size(0) >= percls for c in [0, 1]]):
                    break
        elif hasattr(self.train_set, 'dataset') and isinstance(self.train_set.dataset, ThreeReturnsDataset):
            for xb, yb, refb in loader:
                x, y, ref = torch.cat([x, xb]), torch.cat([y, yb]), torch.cat([ref, refb])
                if all([x[y == c].size(0) >= percls for c in [0, 1]]):
                    break
        else:
            for xb, yb in loader:
                ...
                if all([x[y == c].size(0) >= percls for c in [0, 1]]):
                    break
        found_labels = sorted(set(y.tolist()))
        for c in found_labels:
            out.append(x[y == c][:percls])
        if len(found_labels) < 2:
            out.insert(len(out) - 1 if 0 not in found_labels else len(out), torch.zeros_like(out[-1]))
        if len(gts) > 0:
