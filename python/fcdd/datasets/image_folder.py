...
class ADImageFolderDataset(TorchvisionDataset):
    ...
    def __init__(
        ...
        if preproc in ["", None, "default", "none"]:
            ...
            transform = transforms.Compose(
                [
                    transforms.ColorJitter(
                        brightness=0.01, contrast=0.01, saturation=0.01
                    ),
                    transforms.RandomHorizontalFlip(),
                    transforms.ToTensor(),
                    transforms.Lambda(lambda x: x + 0.001 * torch.randn_like(x)),
    ...
    def balance_dataset(self, gtm=False):
        ...
        anomaly_mask = anomaly_mask * (
            1
            if self.ovr
            else np.isin(self._train_set.targets, np.asarray(self.normal_classes))
        )
...
class ImageFolderDataset(ImageFolder):
    def __init__(
        ...
        if ovr:
            ...
            self.anomaly_labels = [
                nominal_label
                if f.split(os.sep)[-2].lower() in ["normal", "nominal"]
                else anomalous_label
