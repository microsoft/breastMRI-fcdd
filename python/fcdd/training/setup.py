...
def pick_opt_sched(
    net: BaseNet, lr: float, wdk: float, sched_params: List[float], opt: str, sched: str
):
    """
    ...
    if opt == "sgd":
        optimizer = optim.SGD(
            net.parameters(), lr=lr, weight_decay=wdk, momentum=0.9, nesterov=True
        )
    elif opt == "adam":
    ...
    if sched == "lambda":
        ...
        scheduler = optim.lr_scheduler.LambdaLR(
            optimizer, lambda ep: sched_params[0] ** ep
        )
    elif sched == "milestones":
        assert (
            len(sched_params) >= 2
            and 0 < sched_params[0] <= 1
            and all([p > 1 for p in sched_params[1:]])
        )
        scheduler = optim.lr_scheduler.MultiStepLR(
            optimizer,
            [int(s) for s in sched_params[1:]],
            sched_params[0],
        )
    else:
...
def trainer_setup(
    ...
    validation_every_epochs: float = 0.0,
) -> dict:
    """
    ...
    device = torch.device(f"cuda:{gpu}") if cuda else torch.device("cpu")
    ...
    loaders = ds.loaders(batch_size=batch_size, num_workers=workers, validation_ratio=validation_ratio)
    net = load_nets(net, ds.shape, bias=bias)
    logger.logtxt(
        "##### NET RECEPTION {} #####".format(
    ...
    if not hasattr(ds, "nominal_label") or ds.nominal_label < ds.anomalous_label:
        ...
        ds_order = ["anom", "norm"]
    if isinstance(ds.train_set, GTMapADDataset):
        ds_order = [
            *ds_order,
            "",
            *["gtno" if s == "norm" else "gtan" for s in ds_order],
        ]
    elif hasattr(ds.train_set, "dataset") and isinstance(
        ds.train_set.dataset, ThreeReturnsDataset
    ):
        ds_order = [
            *ds_order,
            "",
            *["RFno" if s == "norm" else "RFan" for s in ds_order],
        ]
    images = ds.preview(loaders[0], 20)
    logger.imsave(
        "ds_preview", torch.cat([*images]), nrow=images.size(1), rowheaders=ds_order
    )
    if loaders[1] is not None:
        images = ds.preview(loaders[1], 20)
        logger.imsave(
            "ds_preview_val", torch.cat([*images]), nrow=images.size(1), rowheaders=ds_order
        )
    images = ds.preview(loaders[2], 20,)
    logger.imsave(
        "ds_preview_test", torch.cat([*images]), nrow=images.size(1), rowheaders=ds_order,
    )
    return {
        "net": net,
