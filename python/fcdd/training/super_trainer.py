...
class SuperTrainer(object):
    def __init__(
        ...
        :param validation_every_epochs: if there is a validation split, this defines when to run an iteration of
            the validation data. E.g., `2` results into a validation pass at the end of every second epoch of training.
        """
        if objective in ["fcdd"]:
            assert isinstance(
                net, FCDDNet
            ), "For the FCDD objective, the net needs to be an FCDD net!"
        elif objective in ["hsc"]:
            assert not isinstance(
                net, FCDDNet
            ), "For the HSC objective, the net must not be an FCDD net!"
        elif objective in ["bce"]:
            assert not isinstance(
                net, FCDDNet
            ), "For the BCE objective, the net must not be an FCDD net!"
        elif objective in ["ae"]:
            assert hasattr(
                net, "encoder_cls"
            ), "For the AE objective, the net must be an autoencoder!"
        ...
        elif objective == "fcddrefs":
            self.trainer = FCDDRefsTrainer(
                net,
                opt,
                sched,
                dataset_loaders,
        ...
        elif objective == "fcddrefs_symmetric":  # Adding this here, needs to be checked
            self.trainer = FCDDRefsTrainer(
                net,
                opt,
                sched,
    ...
    def train(self, epochs: int, snap: str = None, acc_batches=1):
        ...
        try:
            ...
            # TO DO, add this here: self.trainer.snapshot(epochs)
        finally:
            self.logger.save()
            self.logger.plot()
    ...
    def test(self, specific_viz_ids: Tuple[List[int], List[int]] = ()) -> dict:
        ...
        if os.path.exists(os.path.join(self.logger.dir, "snapshot_best_val_roc.pt")):
            self.logger.print('---------- Test model for snapshot at best validation ROC.')
            self.load(os.path.join(self.logger.dir, "snapshot_best_val_roc.pt"))
            res_best_val_roc = self.trainer.test(specific_viz_ids, subdir='test_best_val_roc_model')
            if res_best_val_roc is not None:
                for k in list(res_best_val_roc.keys()):
                    res_best_val_roc[f'best_val_{k}'] = res_best_val_roc.pop(k)
                self.res.update(res_best_val_roc)

        if os.path.exists(os.path.join(self.logger.dir, "snapshot_best_val_gtmap_roc.pt")):
            self.logger.print('---------- Test model for snapshot at best validation pixel-wise ROC.')
            self.load(os.path.join(self.logger.dir, "snapshot_best_val_gtmap_roc.pt"))
            res_best_val_gtmap_roc = self.trainer.test(specific_viz_ids, subdir='test_best_val_gtmap_roc_model')
            if res_best_val_gtmap_roc is not None:
                for k in list(res_best_val_gtmap_roc.keys()):
                    res_best_val_gtmap_roc[f'best_val_gtmap_{k}'] = res_best_val_gtmap_roc.pop(k)
                self.res.update(res_best_val_gtmap_roc)

        self.load(os.path.join(self.logger.dir, "snapshot.pt"))
        return self.res
