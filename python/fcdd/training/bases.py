...
def safe_concat(tensor_list, dim=0):
    ...
    # Iterate through tensor_list to update batch sizes and handle zero-dimensional tensors
    for tensor in tensor_list:
        if tensor is None:
            batch_sizes.append(0)
        elif tensor.dim() == 0:
            # Replace zero-dimensional tensor with a tensor containing a single zero
            batch_sizes.append(1)
        else:
            batch_sizes.append(tensor.size(dim))

    # Identify a reference size
    ref_size = next(
        (
            tensor.size()
            for tensor in tensor_list
    ...
    # Create new tensor list with replaced zero-dimensional tensors
    new_tensor_list = [
        torch.zeros(ref_size) if tensor is None or tensor.dim() == 0 else tensor
        for tensor in tensor_list
    ]

    return torch.cat(new_tensor_list, dim=dim)
...
class BaseTrainer(ABC):
    def __init__(
        ...
        opt: Optimizer,
        sched: _LRScheduler,
        dataset_loaders: Tuple[DataLoader, DataLoader, DataLoader],
        logger: Logger,
        device="cuda:0",
        **kwargs,
    ):
    ...
    def load(self, path: str) -> int:
        ...
        print(
            "Loaded {}{}{} with starting epoch {} for {}".format(
                "net_state, " if net_state else "",
                "opt_state, " if opt_state else "",
                "sched_state" if sched_state else "",
                epoch,
                str(self.__class__)[8:-2],
            )
        )
...
class BaseADTrainer(BaseTrainer):
    ...
    def train(self, epochs: int, acc_batches=1) -> BaseNet:
        ...
        for epoch in range(epochs):
            ...
            for n_batch, data in enumerate(self.train_loader):
                ...
                gtmaps, refs, outputs_ref = None, None, None
                if isinstance(self.train_loader.dataset, GTMapADDataset):
                    inputs, labels, gtmaps = data
                    gtmaps = gtmaps.to(self.device)
                elif hasattr(self.train_loader.dataset, "dataset") and isinstance(
                    self.train_loader.dataset.dataset, ThreeReturnsDataset
                ):
                    inputs, labels, refs = data
                ...
                self.opt.zero_grad()
                if refs is not None:
                    outputs, outputs_ref = self.net(inputs, refs)
                    loss = self.loss(outputs, inputs, labels, gtmaps, refs=outputs_ref)
                else:
                    outputs = self.net(inputs)
                ...
                with torch.no_grad():
                    ...
                    if len(set(labels.tolist())) > 1:
                        if refs is not None:
                            ...
                            swloss = self.loss(
                                outputs, inputs, labels, gtmaps, reduce="none"
                            )
    ...
    def val(self, epoch: int):
        ...
        self.logger.print("Compute scores for validation split...", fps=False)
        (
            labels,
            loss,
            anomaly_scores,
            ...
            grads,
            refs,
        ) = self._gather_data(self.val_loader, val=True)
        if len(set(labels)) == 1:
            self.net = self.net.to(self.device).train()
            self.logger.print(
                f"EPOCH {epoch:02d}. Validation split contains no anomalous samples. Cannot compute validation score.",
                fps=False,
                err=True,
            )
            return
        ...
        self.logger.add_scalar("validation_roc", sc["roc"]["auc"], step=epoch)
        if sc["roc"]["auc"] == min(self.logger._scalars["validation_roc"]):
            self.logger.snapshot(
                self.net, self.opt, self.sched, epoch, name="snapshot_best_val_roc"
            )
        if sc.get("gtmap_roc", None) is not None:
            self.logger.add_scalar(
                "validation_gtmap_roc", sc["gtmap_roc"]["auc"], step=epoch
            )
            if sc["gtmap_roc"]["auc"] == min(
                self.logger._scalars["validation_gtmap_roc"]
            ):
                self.logger.snapshot(
                    self.net,
                    self.opt,
                    self.sched,
                    epoch,
                    name="snapshot_best_val_gtmap_roc",
                )
        self.net = self.net.to(self.device).train()

    def test(
        self, specific_viz_ids: Tuple[List[int], List[int]] = (), subdir="."
    ) -> dict:
        """
        ...
        :param subdir: whether to have a subdir where all test data generated here is logged.
    ...
    def _gather_data(
        self, loader: DataLoader, gather_all=False, val=False
    ) -> Tuple[List[int], Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor]:
        ...
        for n_batch, data in enumerate(loader):
            if isinstance(loader.dataset, GTMapADDataset):
                ...
                and isinstance(loader.dataset.dataset, ThreeReturnsDataset)
                or isinstance(loader.dataset, ThreeReturnsDataset)
            ):
                inputs, labels, refs = data
            ...
            inputs = inputs.to(self.device)
            if gather_all:
                outputs, loss, anomaly_score, _ = self._regular_forward(
                    inputs,
                    labels,
                    refs.detach().to(self.device) if refs is not None else None,
                )
                inputs = bk_inputs.clone().to(self.device)
                _, _, _, grads = self._grad_forward(
                    inputs,
                    labels,
                    refs.detach().to(self.device) if refs is not None else None,
                )
            elif self.objective == "hsc":
                outputs, loss, anomaly_score, grads = self._grad_forward(
                    inputs,
                    labels,
                    refs.detach().to(self.device) if refs is not None else None,
                )
            ...
            #     outputs, loss, anomaly_score, grads = self._grad_forward(inputs, labels)
                ...
                outputs, loss, anomaly_score, grads = self._regular_forward(
                    inputs,
                    labels,
                    refs.detach().to(self.device) if refs is not None else None,
                )
        ...
        all_anomaly_scores = safe_concat(
            all_anomaly_scores
        )  # Modified from torch.cat to safe_concat to deal with zero-dimensional tensors
        all_loss = safe_concat(
            all_loss
        )  # Modified from torch.cat to safe_concat to deal with zero-dimensional tensors
        # all_grads = torch.cat(all_grads) if len(all_grads) > 0 else None
        all_grads = (
            safe_concat(all_grads) if len(all_grads) > 0 else None
        )  # Modified from torch.cat to safe_concat to deal with zero-dimensional tensors
        all_refs = torch.cat(all_refs) if len(all_refs) > 0 else None
        ret = (
            all_labels,
            all_loss,
            all_anomaly_scores,
    ...
    def _regular_forward(
        self, inputs: Tensor, labels: Tensor, refs: Tensor = None
    ) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
        with torch.no_grad():
            if refs is not None:
                outputs, outputs_ref = self.net(inputs, refs)
                loss = self.loss(
                    outputs, inputs, labels, reduce="none", refs=outputs_ref
                )
                outputs = outputs[
                    0
                ]  # assuming first image is the one of interest... Outputs are not used alter on anyway...
            else:
                outputs = self.net(inputs)
    ...
    def _grad_forward(
        ...
        if refs is not None:
            outputs, outputs_ref = self.net(inputs, refs)
            loss = self.loss(outputs, inputs, labels, reduce="none", refs=outputs_ref)
            outputs = outputs[
                0
            ]  # assuming first image is the one of interest... Outputs are not used alter on anyway...
        else:
    ...
    def score(
        ...
        if torch.isnan(ascores).sum() > 0:
            self.logger.logtxt(
                "Could not compute test scores, since anomaly scores contain nan values!!!",
                True,
            )
            return None
        ...
        if not val:
            self.logger.single_plot(
                ...
                legend=["auc={}".format(roc_score)],
                subdir=subdir,
            )
            self.logger.single_save("roc", roc_res, subdir=subdir)
            self.logger.logtxt(
                "##### ROC TEST SCORE {} #####".format(roc_score), print=True
            )
        ...
        if gtmaps is not None:
            try:
                ...
                ascores = (
                    self.reduce_pixelwise_ascore(ascores) if not use_grads else grads
                )
                if not val and hasattr(
                    self.test_loader.dataset.dataset, "get_original_gtmaps_normal_class"
                ):
                    gtmaps = (
                        self.test_loader.dataset.dataset.get_original_gtmaps_normal_class()
                    )
                if isinstance(
                    self.net, ReceptiveNet
                ):  # Receptive field upsampling for FCDD nets
                ...
                flat_gtmaps, flat_ascores = (
                    gtmaps.reshape(-1).int().tolist(),
                    ascores.reshape(-1).tolist(),
                )
                ...
                if not val:
                    ...
                    self.logger.single_save("gtmap_roc", gtmap_roc_res, subdir=subdir)
                    self.logger.logtxt(
                        "##### GTMAP ROC TEST SCORE {} #####".format(gt_roc_score),
                        print=True,
                    )
    ...
    def heatmap_generation(
        ...
        if minsamples < 2:
            self.logger.warning(
                f"Heatmap '{name}' cannot be generated. For some labels there are too few samples!",
                unique=False,
            )
        else:
            ...
            for l in sorted(set(labels)):
                idx.extend(
                    (lbls == l).nonzero().squeeze(-1).tolist()[:this_show_per_cls]
                )
            ...
            for l in sorted(set(labels)):
                ...
                sort = [
                    i
                    for i in np.argsort(
                        rascores.detach().reshape(rascores.size(0), -1).sum(1)
                    ).tolist()
                    if i in lid
                ]
        ...
        if "train" not in name:
            res = (
                self.resdown * 2
            )  # increase resolution limit because there are only a few heatmaps shown here
            rascores = self.reduce_ascore(ascores)
            inpshp = imgs.shape
            for l in sorted(set(labels)):
                lid = set(
                    (torch.from_numpy(np.asarray(labels)) == l)
                    .nonzero()
                    .squeeze(-1)
                    .tolist()
                )
                ...
                self.logger.logtxt(
                    "Interpretation visualization paper image {} indicies for label {}: {}".format(
                        "{}_paper_lbl{}".format(name, l), l, idx
                    )
                )
                ...
                if specific_idx is not None and len(specific_idx) > 0:
                    self._create_singlerow_heatmaps_picture(
                        ...
                        inpshp,
                        l,
                        pt.join(subdir, "specific_viz_ids"),
                        res,
                        imgs,
                        ascores,
                        grads,
    ...
    def _create_heatmaps_picture(
        ...
        for s in range(number_of_rows):
            rows.append(
                self._image_processing(
                    imgs[idx][s * nrow : s * nrow + nrow],
                    inpshp,
                    maxres=self.resdown,
                    qu=1,
                )
            )
            if self.objective != "hsc" and self.objective != "bce":
                rows.append(
                    self._image_processing(
        ...
        self.logger.imsave(
            name, torch.cat(rows), nrow=nrow, scale_mode="none", subdir=subdir
        )

    def _create_singlerow_heatmaps_picture(
        self,
        idx: List[int],
        name: str,
        inpshp: torch.Size,
        ...
        for norm in ["local", "global"]:
            ...
            if gtmaps is not None:
                rows.append(
                    self._image_processing(gtmaps[idx], inpshp, maxres=res, norm=None)
                )
            tim = torch.cat(rows)
            imname = "{}_paper_{}_lbl{}".format(name, norm, lbl)
            self.logger.single_save(
                imname, torch.stack(rows), subdir=pt.join("tims", subdir)
            )
            self.logger.imsave(
                imname, tim, nrow=len(idx), scale_mode="none", subdir=subdir
            )
    ...
    def _image_processing(
        ...
        if blur:
            ...
            elif self.objective == "hsc" or self.objective == "bce":
        ...
        if norm is not None:
            apply_norm = {"local": self.__local_norm, "global": self.__global_norm}
    ...
    def __local_norm(imgs: Tensor, qu: int, ref: Tensor = None) -> Tensor:
        ...
        """
        imgs.sub_(
            imgs.reshape(imgs.size(0), -1).min(1)[0][
                (...,) + (None,) * (imgs.dim() - 1)
            ]
        )
