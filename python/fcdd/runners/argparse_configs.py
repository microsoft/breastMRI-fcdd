...
class DefaultConfig(object):
    def __call__(self, parser: ArgumentParser) -> ArgumentParser:
        ...
        parser.add_argument(
            "--logdir",
            type=str,
            default=pt.join("..", "..", "data", "results", "fcdd_{t}"),
        ...
        parser.add_argument(
            "-d", "--dataset", type=str, default="custom", choices=DS_CHOICES
        )
        parser.add_argument(
            "--validation-ratio", type=float, default=0.0,
            help="Defines the ratio of validation data that is split from the training data. Defaults to 0.0, which "
                 "disables validation. The model is evaluated on the validation data at the end of every epoch. "
                 "Note that the test data is unaffected by this parameter."
        )
        parser.add_argument(
            "--validation-every-epochs", type=int, default=1,
            help="Defines when to run the validation."
        )
        parser.add_argument(
            "-n",
            "--net",
            type=str,
            default="FCDD_CNN224_VGG_F",
            choices=choices(),
        ...
        parser.add_argument(
            ...
            dest="bias",
            action="store_false",
            help="Uses no bias in network layers.",
        )
        parser.add_argument(
            "--cpu", dest="cuda", action="store_false", help="Trains on CPU only."
        )
        ...
        parser.set_defaults(
            cuda=True, bias=True, blur_heatmaps=False, online_supervision=True
        )
        return parser
