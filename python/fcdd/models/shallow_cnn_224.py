...
class CNN224_RESNET_NOPT(BaseNet):
    fcdd_cls = FCDD_CNN224_VGG_NOPT
...
class AE224(BaseNet):
    ...
    def __init__(self, encoder, **kwargs):
        ...
        self.deconv1 = nn.ConvTranspose2d(
            int(784 / (7 * 7)), 64, 3, bias=self.bias, padding=1
        )
        ...
        nn.init.xavier_uniform_(
            self.deconv6.weight, gain=nn.init.calculate_gain("leaky_relu")
        )
        self.bn2dd6 = nn.BatchNorm2d(8, eps=1e-04, affine=self.bias)

        self.deconv7 = nn.ConvTranspose2d(
            8, self.encoder.in_shape[0], 5, bias=self.bias, padding=2
        )
        nn.init.xavier_uniform_(
            self.deconv7.weight, gain=nn.init.calculate_gain("leaky_relu")
        )
